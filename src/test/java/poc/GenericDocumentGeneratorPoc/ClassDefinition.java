...
public class ClassDefinition {
    ...
    private List<AttributeDefinition> attributeDefinition;
    ...
    public void addAttributeDefinition(AttributeDefinition definition){
        if ( alreadyExistsSame(definition)){
            String msg = String.format( "Duplicate attibute definition for class, class name: %s, code: %s are not allowed in the set (per one class of document)" , definition.getAtrubuteClass( ) , definition.getAttributeCode());
    ...
    private boolean alreadyExistsSame(AttributeDefinition definition) {
        return attributeDefinition.stream( ).anyMatch( e -> e.getAtrubuteClass( ).equals( definition.getAtrubuteClass( ) )

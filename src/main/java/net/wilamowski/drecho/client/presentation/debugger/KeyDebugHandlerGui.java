...
public class KeyDebugHandlerGui implements DebugHandler {
  ...
  private void setKeyCombination() {
    String keyCombinationPropertyValue =
        ClientPropertyReader.getString("user.ui.debug.key-invoke-modal");
    customKeyCombination =
        keyCombinationPropertyValue.isEmpty()
            ? DEFAULT_KEY_COMBINATION
            : keyCombinationPropertyValue;
  ...
  private void handleDebugKey() {
    ...
    try {
      String toStrings =
          Arrays.stream(observableObjects)

...
public class PatientServiceStandalone implements PatientService {
  ...
  @Override
  public List<PatientDto> findByCitizenCode(String param, int page) {
  ...
  private List<PatientDto> getPatients(String param, int page) {
    ...
    if (param.isBlank()) {
      ...
      if (StringUtils.isNumeric(param)) {
        ...
        List<Patient> byPeselCode = patientRepository.findByCitizenCode( param , page );
  ...
  public List<PatientDto> findByFullName(String param, int page) {
    ...
    if (StringUtils.isAlpha(trimedParam ) ) {
      logger.debug("[SERVICE] Parameter  is alpha. Returning all patient by last name");
      List<Patient> byLastName = patientRepository.findByFullName(trimedParam, page);
    ...
    } else if (trimedParam.contains(" ")) {
      logger.debug("[SERVICE] Parameter contain white char. Returning all patient by full name");
      List<Patient> byLastName = patientRepository.findByFullName(trimedParam, page);
      logger.debug("[SERVICE] Founded patients number: {}", byLastName.size());
      return byLastName.stream().map( PatientDomainDtoMapper::toDto ).collect( Collectors.toList());
    } else if (StringUtils.isNumeric( trimedParam )) {
      logger.debug("[SERVICE] Parameter contain white char. Returning all patient by full name");
      List<Patient> byLastName = patientRepository.findByCitizenCode(trimedParam, page);
      logger.debug("[SERVICE] Founded patients number: {}", byLastName.size());
      return byLastName.stream().map( PatientDomainDtoMapper::toDto ).collect( Collectors.toList());
    } else {
      logger.debug("[SERVICE] Parameter {} not handled. Return empty list.", trimedParam);
      return Collections.emptyList();
    }
  }
  ...
  @Override
  public int counterByCitizenCode(String code) {
    logger.trace("[SERVICE] Entering counterByFullName...");
    return patientRepository.countByCitizenCode(code);
  }
  ...
  @Override
  public PatientDto findByCitizenCodeFirstRecord(String par) {
    List<PatientDto> patients = this.findByCitizenCode(par, 0);

    if (patients.isEmpty()) {
      logger.warn("[SERVICE] No patient found for citizen code: {}", par);
      return null;
    }
